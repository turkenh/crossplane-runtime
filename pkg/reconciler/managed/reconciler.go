...
func (r *Reconciler) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) { //nolint:gocyclo // See note below.
	...
	if meta.WasDeleted(managed) {
		...
		if meta.IsExternalDeleteBlocked(managed) {
			log.Debug("External deletion is blocked by another resource", "blocking-resources", managed.GetAnnotations()[meta.AnnotationKeyExternalDeleteBlocked])
			record.Event(managed, event.Normal(reasonPending, "External deletion is blocked until the depending resource is deleted"))
